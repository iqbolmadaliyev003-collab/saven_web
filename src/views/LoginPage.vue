...
<template>
  <div class="login-bg flex min-h-screen items-center justify-center p-4">
    <div class="w-full max-w-md rounded-3xl bg-card p-8 shadow-2xl">
      <div
        class="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary text-primary-foreground text-2xl font-bold">
        S
      </div>
      ...
      <form class="mt-6 space-y-4" @submit.prevent="handleSubmit">
        <div class="space-y-1.5">
          ...
          <input id="login" v-model="login" placeholder="example@biznes.uz"
        ...
        <div class="space-y-1.5">
          ...
          <div class="relative">
            ...
            <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted text-xs"
              @click="showPassword = !showPassword">
              {{ showPassword ? "Yashirish" : "Ko'rsatish" }}
            </button>
          </div>
          <div class="flex justify-end">
            <button type="button" class="text-xs font-medium text-success"
    ...
    <div v-if="helpOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
      ...
      <div class="w-full max-w-sm rounded-3xl bg-card p-6 text-center shadow-2xl">
        <div
          class="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary text-primary-foreground text-2xl">
          ?
        </div>
        ...
        <button class="mx-auto mt-4 flex items-center gap-2 rounded-full bg-secondary px-4 py-3 shadow-sm"
          @click="copyPhone">
          <span
            class="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-primary-foreground">☎</span>
